...
{
	float planetRadius = atmosphere.bottomRadius * SKY_UNIT_TO_M * 0.0254;
	float3 planetCenterWorld = atmosphere.planetCenter * SKY_UNIT_TO_M * 0.0254;
...
void main(uint3 DTid : SV_DispatchThreadID)
{
	...
	// use Z=1 to get the near plane instead of far plane for more accuracy
	float4 unprojected = mul(g_xCamera_InvP, float4(screenPosition, 1, 1));
	unprojected.xyz /= unprojected.w;

	// use 3x3 matrix to avoid adding and removing the camera position
	float3x3 InvView3 = (float3x3) g_xCamera_InvV;
	float3 direction = mul( InvView3, unprojected.xyz );

	float3 rayOrigin = g_xCamera_CamPos;
	float3 rayDirection = normalize( direction );
	...
	float stepSize;
    {
		...
		float planetRadius = parameters.bottomRadius * SKY_UNIT_TO_M * 0.0254;
		float3 planetCenterWorld = parameters.planetCenter * SKY_UNIT_TO_M * 0.0254;
