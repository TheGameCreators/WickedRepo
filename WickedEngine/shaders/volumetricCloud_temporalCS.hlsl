...
static const float temporalScale = 4.0;
...
float getInverseLinearDepth(float lin, float near, float far)
{
	...
	// simplified
	float temp = (lin * near) / far; // if far goes to infinity then this goes to 0
	return (near - temp) / (lin - temp);
}
...
void main(uint3 DTid : SV_DispatchThreadID)
{
	...
	float x = uv.x * 2 - 1;
	float y = (1 - uv.y) * 2 - 1;
    float4 prevPos = float4(x, y, 1.0, 1.0);
    prevPos = mul(g_xCamera_InvP, prevPos);
    prevPos = prevPos / prevPos.w;
	
    prevPos = mul(g_xCamera_InvV, prevPos);
    prevPos = mul(g_xCamera_PrevV, prevPos);
	
    float4 reproj = mul(g_xCamera_Proj, prevPos);
    reproj /= reproj.w;
	
    float2 prevUV = reproj.xy * float2(0.5, -0.5) + 0.5;
