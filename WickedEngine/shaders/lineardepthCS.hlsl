...
void main(uint3 DTid : SV_DispatchThreadID, uint3 GTid : SV_GroupThreadID)
{
	...
	output_depth_mip1[DTid.xy] = depths.x;
	...
	if (GTid.x % 2 == 0 && GTid.y % 2 == 0)
	{
		...
		//tile[GTid.x][GTid.y] = maxdepth;
		output_depth_mip2[DTid.xy / 2] = depths.x;
	}
	//GroupMemoryBarrierWithGroupSync();
	...
	//tile[GTid.x][GTid.y] = maxdepth;
	output_lineardepth_mip1[DTid.xy] = lineardepths.x;
	//GroupMemoryBarrierWithGroupSync();
	...
	if (GTid.x % 2 == 0 && GTid.y % 2 == 0)
	{
		//maxdepth = max(tile[GTid.x][GTid.y], max(tile[GTid.x + 1][GTid.y], max(tile[GTid.x][GTid.y + 1], tile[GTid.x + 1][GTid.y+ 1])));
		//tile[GTid.x][GTid.y] = maxdepth;
		output_lineardepth_mip2[DTid.xy / 2] = lineardepths.x;
	}
	//GroupMemoryBarrierWithGroupSync();
	...
	if (GTid.x % 4 == 0 && GTid.y % 4 == 0)
	{
		...
		//tile[GTid.x][GTid.y] = maxdepth;
		output_lineardepth_mip3[DTid.xy / 4] = lineardepths.x;
	}
	//GroupMemoryBarrierWithGroupSync();
	...
	if (GTid.x % 8 == 0 && GTid.y % 8 == 0)
	{
		...
		//tile[GTid.x][GTid.y] = maxdepth;
		output_lineardepth_mip4[DTid.xy / 8] = lineardepths.x;
	}
	//GroupMemoryBarrierWithGroupSync();
	...
	if (GTid.x % 16 == 0 && GTid.y % 16 == 0)
	{
		...
		output_lineardepth_mip5[DTid.xy / 16] = lineardepths.x;
	}
}
