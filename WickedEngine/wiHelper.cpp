...
namespace wiHelper
{
	...
	bool loadTextureFromMemory(const wiGraphics::Texture& texture, std::vector<uint8_t>& texturedata)
	{
		...
		staging_desc.CPUAccessFlags = CPU_ACCESS_WRITE;
		...
		Mapping mapping;
		mapping._flags = Mapping::FLAG_WRITE;
		...
		if (mapping.data != nullptr)
		{
			if (mapping.rowpitch / data_stride != desc.Width)
			{
				...
				for (uint32_t i = 0; i < desc.Height; ++i)
				{
					...
					void* dst = (void*)((size_t)mapping.data + size_t(i * mapping.rowpitch));
			...
			{
				// Copy whole
				std::memcpy(mapping.data, texturedata.data(), texturedata.size());
			}
		...
		{
			GPUBarrier barriers[] = {
				GPUBarrier::Image(&texture, texture.desc.layout, IMAGE_LAYOUT_COPY_DST, 0)
			};
		...
		{
			GPUBarrier barriers[] = {
				GPUBarrier::Image(&texture, IMAGE_LAYOUT_COPY_DST, texture.desc.layout, 0)
			};
			device->Barrier(barriers, arraysize(barriers), cmd);
		}
	...
	void convertTextureToRGBA(const std::vector<uint8_t>& texturedata, const wiGraphics::TextureDesc& desc, bool bBGRAOutput)
	{
		...
		else if (desc.Format == FORMAT_B8G8R8A8_UNORM)
		{
			// GGREDUCED need to save out RGBA format (so convert from BGRA)
			uint32_t* data32 = (uint32_t*)texturedata.data();
			for (uint32_t i = 0; i < data_count; ++i)
			{
				uint32_t pixel = data32[i];
		...
		// GGREDUCED and finally if flagged, convert RGBA to BGRA
		if (bBGRAOutput == true)
		{
			uint32_t* data32 = (uint32_t*)texturedata.data();
			for (uint32_t i = 0; i < data_count; ++i)
	...
	bool saveTextureToMemoryFile(const std::vector<uint8_t>& texturedata, const wiGraphics::TextureDesc& desc, const std::string& fileExtension, std::vector<uint8_t>& filedata)
	{
		...
		convertTextureToRGBA(texturedata, desc, false);
		/* GGREDUCED useful, so made a function of it
		uint32_t data_count = desc.Width * desc.Height;
		if (desc.Format == FORMAT_R10G10B10A2_UNORM)
		{
			// So this should be converted first to rgba8 before saving to common format...
		...
		{
			// GGREDUCED need to save out BGRA format
			uint32_t* data32 = (uint32_t*)texturedata.data();
			for (uint32_t i = 0; i < data_count; ++i)
			{
				uint32_t pixel = data32[i];
	...
	int GetBitsPerPixel(FORMAT value)
	{
		switch (value)
		{
		case FORMAT_UNKNOWN:
	...
	const char* GetFormatName(FORMAT value)
	{
		switch (value)
		{
		case FORMAT_UNKNOWN:
