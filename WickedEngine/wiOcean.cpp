...
void wiOcean::Create(const OceanParameters& params)
{
	...
	for (uint32_t i = 0; i < displacementMap.GetDesc().MipLevels; ++i)
	{
		int subresource_index;
		subresource_index = device->CreateSubresource(&displacementMap, SRV, 0, 1, i, 1);
		assert(subresource_index == i);
		subresource_index = device->CreateSubresource(&displacementMap, UAV, 0, 1, i, 1);
		assert(subresource_index == i);
	}
...
void wiOcean::UpdateDisplacementMap(const OceanParameters& params, CommandList cmd) const
{
	...
	wiRenderer::GenerateMipChain(displacementMap, wiRenderer::MIPGENFILTER_LINEAR, cmd);
...
void wiOcean::Render(const CameraComponent& camera, const OceanParameters& params, CommandList cmd) const
{
	...
	Ocean_RenderCB cb;
	cb.xOceanWaterColor = XMFLOAT3( params.waterColor.x, params.waterColor.y, params.waterColor.z );
	...
	cb.xOceanFogMin = params.fogMinDist;
